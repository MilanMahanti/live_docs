...
export const getAllDocuments = async (email: string) => {
  try {
    const allDocument = await liveblocks.getRooms({ userId: email });
    return parseStringify(allDocument);
  } catch (error) {
    console.log("Error happend while getting all the rooms");
  }
};
...
export const updateDocumentTitle = async (roomId: string, title: string) => {
  try {
    ...
    console.log("Something wrong happed updating room");
  }
};
...
export const updateDocumentAccess = async ({
  ...
  try {
    ...
    console.log(`Error happed while updating a room access: ${error}`);
  }
};
...
export const removeCollaborator = async ({
  ...
  try {
    ...
    console.log(`Error happed while removing collaborator: ${error}`);
  }
};
